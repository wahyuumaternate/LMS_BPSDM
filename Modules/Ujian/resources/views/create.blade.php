...
@section('page-title', 'Tambah Ujian Baru')
...
@section('content')
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Form Tambah Ujian Baru</h5>
                                ...
                                <select class="form-select @error('kursus_id') is-invalid @enderror" id="kursus_id"
                                    ...
                                    @foreach (\Modules\Kursus\Entities\Kursus::orderBy('judul')->get() as $kursus)
                                        <option value="{{ $kursus->id }}"
                                            {{ old('kursus_id', request('kursus_id')) == $kursus->id ? 'selected' : '' }}>
                                            {{ $kursus->judul }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control @error('judul_ujian') is-invalid @enderror"
                                    id="judul_ujian" name="judul_ujian" value="{{ old('judul_ujian') }}" required>
                                @error('judul_ujian')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-4">
                                <input type="number" step="0.01"
                                    ...
                                    name="bobot_nilai" value="{{ old('bobot_nilai', 1.0) }}" min="0.1" max="100"
                                    required>
                                @error('bobot_nilai')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <textarea class="form-control @error('aturan_ujian') is-invalid @enderror" id="aturan_ujian" name="aturan_ujian"
                                    rows="4">{{ old('aturan_ujian') }}</textarea>
                                @error('aturan_ujian')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
